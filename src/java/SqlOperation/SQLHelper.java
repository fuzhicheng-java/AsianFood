...
{  
     ...
     * 连接字符串 
     */  
    public static String url="jdbc:mysql://localhost:3306/asianfood";  
    /** 
     * 用户名 
     */  
     ...
     * 密码 
     */  
    public static String password="333444";  
    /** 
     * 不允许实例化该类 
     */ 
...
//    } 
     ...
     * @return 非负数:正常执行; -1:执行错误; -2:连接错误 
     */  
    public static int ExecSqlQuery(String cmdText)  
    {  
        Statement stmt = getStatement();  
        if (stmt == null)  
        {  
        ...
        try  
        {  
            if(stmt.executeQuery(cmdText)!=null) 
            {
                i=1;
            }
            else
     ...
     * @return 非负数:正常执行; -1:执行错误; -2:连接错误 
     */  
    public static int ExecSqlQuery(Connection conn, String cmdText)  
    {  
        Statement stmt = getStatement(conn);  
        if (stmt == null)  
        {  
     ...
     * @return 非负数:正常执行; -1:执行错误; -2:连接错误 
     */  
    public static int ExecSqlQuery(String cmdText, Object... cmdParams) 
    {  
        PreparedStatement pstmt = getPreparedStatement(cmdText, cmdParams);  
        if (pstmt == null)  
        {  
     ...
     * @return 非负数:正常执行; -1:执行错误; -2:连接错误 
     */  
    public static int ExecSqlQuery(Connection conn, String cmdText, Object... cmdParams)  
    {  
        PreparedStatement pstmt = getPreparedStatement(conn, cmdText, cmdParams);  
        if (pstmt == null)  
        {  
